...
def read_nii_files(directory):
    ...
    """
    if os.path.exists("nii_files.txt"):
        with open("nii_files.txt", "r") as f:
            nii_files = [line.strip() for line in f.readlines()]
    ...
    # save the list of NIfTI files to a text file
    with open("nii_files.txt", "w") as f:
...
def process_file(file_path):
    ...
    save_folder = "/mnt/datalake/DS-lake/vankhoa/CT-RATE/dataset/train_preprocessed/" #save folder for preprocessed
    ...
    try:
        row = df[df['VolumeName'] == file_name.replace(".npz", ".nii.gz")]
...
if __name__ == "__main__":
    split_to_preprocess = '/mnt/datalake/DS-lake/vankhoa/CT-RATE/data_volumes/dataset/train' #select the validation or test split
    nii_files = read_nii_files(split_to_preprocess)

    df = pd.read_csv("/mnt/datalake/DS-lake/vankhoa/CT-RATE/dataset/metadata/train_metadata.csv") #select the metadata

    num_workers = 24  # Number of worker processes
